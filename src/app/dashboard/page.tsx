...
export default function Home() {
  ...
  const fetchMovies = async () => {
    ...
    try {
      ...
      setMovies(Array.isArray(data) ? data.filter((m: any) => m.status === 'ToWatch') : []);
    } catch (e) {
  ...
  const handleMarkWatched = async (id: string) => {
    await fetch(`/api/movies/${id}`, {
      ...
      body: JSON.stringify({ status: 'Watched', watchedAt: new Date() })
    });
    fetchMovies();
  };
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
          <h3 className="text-xl font-medium text-slate-500 mb-4">Your watchlist is empty</h3>
          <p className="text-slate-400 mb-6">Start building your collection.</p>
        </div>
